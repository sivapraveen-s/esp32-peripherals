    ...
    while (1) {
        ...
        */
        ...
        if (ret == ESP_ERR_TIMEOUT) {
            ...
            printf("*******************\n");
            printf("TASK[%d]  MASTER READ FROM SLAVE Count-1\n", task_idx);
            printf("*******************\n");
            printf("====TASK[%d] Slave buffer data Count-1 ====\n", task_idx);
            disp_buf(data, d_size);
            printf("====TASK[%d] Master read Count-1====\n", task_idx);
       ...
       // memset(data_wr, 0, 128*sizeof(data_wr[0]));
        ...
        if (ret == ESP_ERR_TIMEOUT) {
            ...
            printf("*******************\n");
            printf("TASK[%d]  MASTER WRITE TO SLAVE Count-2\n", task_idx);
            printf("*******************\n");
            printf("----TASK[%d] Master writeCount-2 ----\n", task_idx);
            disp_buf(data_wr, RW_TEST_LENGTH);
            printf("----TASK[%d] Slave read:Count-2 [%d] bytes ----\n", task_idx, size);
            disp_buf(data, size);
        } else {
